...
from backend.services.federated_service import (
    update_global_model
)
...
def join_job(
    ...
    existing = db.query(OrgJob).filter(
        OrgJob.org_id == org_id,
        OrgJob.job_id == job_id
    ).first()
...
def train_local(
    ...
    if not oj:
        ...
        raise HTTPException(
            status_code=403,
            detail="Must join job before training"
        )
    ...
    org_path = (
        f"backend/storage/local_models/"
        f"org_{org_id}"
    )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to save local model: {e}"
        )
    ...
    try:
        ...
        print(
            f"⚠️ Blockchain logging failed: {e}"
        )
    ...
    return {
        "message": (
            "Training done & "
            "global model updated incrementally"
        ),
...
def download_local(
    ...
    return FileResponse(
        path,
        filename=f"local_model_job_{job_id}.pth"
    )
...
def download_global(
    ...
    if (
        user["role"] != "ADMIN"
        and
        not user.get("can_download_global")
    ):
    ...
    return FileResponse(
        global_model_path,
        filename=f"global_model_job_{job_id}.pth"
    )